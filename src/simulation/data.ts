...
export const RACE_BUILDING_COSTS: Record<Race, Record<BuildingType, { gold: number; wood: number; meat: number; hp: number }>> = {
  ...
  [Race.Horde]: {
    [BuildingType.MeleeSpawner]:  { gold: 0,   wood: 0,  meat: 35, hp: 350 },
    [BuildingType.RangedSpawner]: { gold: 0,   wood: 40, meat: 0,  hp: 300 },
    [BuildingType.CasterSpawner]: { gold: 75,  wood: 0,  meat: 0,  hp: 250 },
...
export const RACE_UPGRADE_COSTS: Record<Race, { tier1: { gold: number; wood: number; meat: number; deathEssence?: number; souls?: number }; tier2: { gold: number; wood: number; meat: number; deathEssence?: number; souls?: number } }> = {
  [Race.Crown]:    { tier1: { gold: 36,  wood: 0,  meat: 0 },  tier2: { gold: 62,  wood: 20, meat: 0 } },
...
// Harvester yields per trip (must match SimHarvesters.ts tickHarvesters)
...
export const UNIT_STATS: Record<Race, RaceUnits> = {
  ...
  [Race.Crown]: {
    [BuildingType.MeleeSpawner]: {
      name: 'Swordsman', hp: 120, damage: 13, attackSpeed: 1.1, moveSpeed: 3.5, range: 1, ascii: '[+]',
    },
    [BuildingType.RangedSpawner]: {
      name: 'Bowman', hp: 38, damage: 13, attackSpeed: 1.3, moveSpeed: 3.5, range: 7, ascii: '>>',
    },
  ...
  [Race.Horde]: {
    [BuildingType.MeleeSpawner]: {
      name: 'Brute', hp: 140, damage: 15, attackSpeed: 1.1, moveSpeed: 3.2, range: 1, ascii: '[#]',
    },
    [BuildingType.RangedSpawner]: {
      name: 'Bowcleaver', hp: 76, damage: 18, attackSpeed: 1.3, moveSpeed: 3.0, range: 7, ascii: '=>',
    },
    [BuildingType.CasterSpawner]: {
      name: 'War Chanter', hp: 64, damage: 15, attackSpeed: 2.0, moveSpeed: 3.2, range: 7, ascii: '{H}',
    },
  },
  ...
  [Race.Wild]: {
    [BuildingType.MeleeSpawner]: {
      name: 'Lurker', hp: 90, damage: 9, attackSpeed: 1.0, moveSpeed: 4.25, range: 1, ascii: '%#',
    },
    [BuildingType.RangedSpawner]: {
  ...
  [Race.Geists]: {
    [BuildingType.MeleeSpawner]: {
      name: 'Bone Knight', hp: 78, damage: 8, attackSpeed: 1.1, moveSpeed: 3.5, range: 1, ascii: '~^',
    },
    [BuildingType.RangedSpawner]: {
  ...
  [Race.Tenders]: {
    [BuildingType.MeleeSpawner]: {
      name: 'Treant', hp: 170, damage: 8, attackSpeed: 1.2, moveSpeed: 2.8, range: 1, ascii: '|T|',
    },
    [BuildingType.RangedSpawner]: {
      name: 'Tinker', hp: 33, damage: 13, attackSpeed: 1.1, moveSpeed: 4.0, range: 7, ascii: '.>',
    },
    [BuildingType.CasterSpawner]: {
      name: 'Grove Keeper', hp: 50, damage: 14, attackSpeed: 2.2, moveSpeed: 3.0, range: 7, ascii: '{Y}',
    },
  },
};
...
export const UPGRADE_TREES: Record<Race, Partial<Record<BuildingType, Record<UpgradeNode, UpgradeNodeDef>>>> = {
  ...
  [Race.Crown]: {
    ...
    [BuildingType.MeleeSpawner]: {
      B: { name: 'Buccaneer', desc: '+20% dmg, +3 gold/kill', damageMult: 1.20, special: { goldOnKill: 3 }, spawnSpeedMult: 0.88, cost: { gold: 40, wood: 0, meat: 0 } },
      C: { name: 'Noble', desc: '+20% speed, faster atk', moveSpeedMult: 1.20, attackSpeedMult: 0.85, spawnSpeedMult: 0.88, cost: { gold: 12, wood: 16, meat: 0 } },
      D: { name: 'Corsair Captain', desc: '+40% HP, +5 gold/death', hpMult: 1.40, special: { goldOnDeath: 5, goldOnKill: 3 }, spawnSpeedMult: 0.82, cost: { gold: 60, wood: 0, meat: 0 } },
      E: { name: 'Pirate King', desc: '+35% dmg, +6 gold/kill', damageMult: 1.35, special: { goldOnKill: 6, goldOnDeath: 8 }, spawnSpeedMult: 0.82, cost: { gold: 72, wood: 16, meat: 0 } },
      F: { name: 'King', desc: '+15% dmg, +30% speed, 30% dodge', damageMult: 1.15, moveSpeedMult: 1.30, special: { dodgeChance: 0.30 }, spawnSpeedMult: 0.82, cost: { gold: 24, wood: 32, meat: 0 } },
      G: { name: 'Champion', desc: '+50% dmg, faster atk', damageMult: 1.50, attackSpeedMult: 0.80, spawnSpeedMult: 0.82, cost: { gold: 16, wood: 44, meat: 0 } },
    },
    // Ranged B-path = wood-heavy, C-path = gold-heavy (scout→siege)
    [BuildingType.RangedSpawner]: {
      B: { name: 'Heavy Bow', desc: '+30% HP, +25% dmg', hpMult: 1.30, damageMult: 1.25, spawnSpeedMult: 0.88, cost: { gold: 8, wood: 20, meat: 0 } },
      C: { name: 'Dwarfette Scout', desc: '+15% speed, faster atk', moveSpeedMult: 1.15, attackSpeedMult: 0.80, spawnSpeedMult: 0.88, cost: { gold: 36, wood: 0, meat: 0 } },
      D: { name: 'Longbow', desc: '+40% dmg, +25% range, slow', damageMult: 1.40, rangeMult: 1.25, spawnSpeedMult: 0.82, special: { extraSlowStacks: 1 }, cost: { gold: 16, wood: 40, meat: 0 } },
      E: { name: 'War Bow', desc: '+35% dmg, splash 2t', damageMult: 1.35, special: { splashRadius: 2, splashDamagePct: 0.50 }, spawnSpeedMult: 0.82, cost: { gold: 20, wood: 36, meat: 0 } },
      F: { name: 'Dwarfette Blitzer', desc: 'Much faster, +25% range', attackSpeedMult: 0.70, rangeMult: 1.25, spawnSpeedMult: 0.82, cost: { gold: 68, wood: 12, meat: 0 } },
      G: { name: 'Cannon', desc: 'SIEGE: 13 range, slow, fragile, devastating vs buildings', hpMult: 0.50, damageMult: 2.16, attackSpeedMult: 3.40, moveSpeedMult: 0.38, rangeMult: 1.85, spawnSpeedMult: 0.82, special: { isSiegeUnit: true, buildingDamageMult: 4.0, splashRadius: 3, splashDamagePct: 0.65 }, cost: { gold: 80, wood: 16, meat: 0 } },
    },
    // Caster B-path = gold-heavy (shields), C-path = wood-heavy (damage mage)
    [BuildingType.CasterSpawner]: {
      B: { name: 'High Priest', desc: '+30% HP, +20% dmg, shield +2 targets', hpMult: 1.30, damageMult: 1.20, special: { shieldTargetBonus: 2 }, spawnSpeedMult: 0.88, cost: { gold: 36, wood: 4, meat: 0 } },
      C: { name: 'War Mage', desc: 'AoE damage, +40% dmg', damageMult: 1.40, special: { crownMage: true, aoeRadiusBonus: 1 }, spawnSpeedMult: 0.88, cost: { gold: 8, wood: 20, meat: 0 } },
      D: { name: 'Arch Bishop', desc: '+50% HP, +25% dmg, shield +3 targets', hpMult: 1.50, damageMult: 1.25, special: { shieldTargetBonus: 3 }, spawnSpeedMult: 0.82, cost: { gold: 68, wood: 8, meat: 0 } },
      E: { name: 'War Cleric', desc: '+50% dmg, shield +25 absorb', damageMult: 1.50, special: { shieldAbsorbBonus: 25 }, spawnSpeedMult: 0.82, cost: { gold: 56, wood: 20, meat: 0 } },
      F: { name: 'Battle Magus', desc: '-30% range, +80% HP, +30% dmg, shields self, burn', rangeMult: 0.70, hpMult: 1.80, damageMult: 1.30, special: { crownMage: true, aoeRadiusBonus: 1, extraBurnStacks: 1, shieldSelf: true }, spawnSpeedMult: 0.82, cost: { gold: 16, wood: 40, meat: 0 } },
      G: { name: 'Archmage', desc: '+80% dmg, large AoE, burn', damageMult: 1.80, rangeMult: 1.25, special: { crownMage: true, aoeRadiusBonus: 3, extraBurnStacks: 2 }, spawnSpeedMult: 0.82, cost: { gold: 24, wood: 48, meat: 0 } },
    },
    // Tower B-path = gold-heavy (fortification), C-path = wood-heavy (range/speed)
    [BuildingType.Tower]: {
      B: { name: 'Reinforced Tower', desc: '+60% HP, +30% dmg', hpMult: 1.60, damageMult: 1.30, cost: { gold: 40, wood: 4, meat: 0 } },
      C: { name: 'Rapid Tower', desc: 'Faster atk, +range', attackSpeedMult: 0.75, special: { towerRangeBonus: 1 }, cost: { gold: 12, wood: 16, meat: 0 } },
      D: { name: 'Fortress Tower', desc: '+150% HP, +range', hpMult: 2.50, special: { towerRangeBonus: 2 }, cost: { gold: 68, wood: 12, meat: 0 } },
      E: { name: 'War Tower', desc: '+50% dmg, +2 range', damageMult: 1.50, special: { towerRangeBonus: 2 }, cost: { gold: 56, wood: 24, meat: 0 } },
      F: { name: 'Gatling Tower', desc: 'Very fast, +range', attackSpeedMult: 0.60, special: { towerRangeBonus: 2 }, cost: { gold: 20, wood: 36, meat: 0 } },
      G: { name: 'Siege Tower', desc: '+60% dmg, +3 range', damageMult: 1.60, special: { towerRangeBonus: 3 }, cost: { gold: 24, wood: 40, meat: 0 } },
    },
  },
  ...
  [Race.Horde]: {
    [BuildingType.MeleeSpawner]: {
      ...
      B: { name: 'Iron Brute', desc: '+50% HP, +20% dmg', hpMult: 1.50, damageMult: 1.20, spawnSpeedMult: 0.88, cost: { gold: 0, wood: 0, meat: 45 } },
    ...
    [BuildingType.CasterSpawner]: {
      ...
      C: { name: 'War Drummer', desc: '+25% dmg, faster atk, +25% range', damageMult: 1.25, attackSpeedMult: 0.80, rangeMult: 1.25, spawnSpeedMult: 0.88, cost: { gold: 0, wood: 45, meat: 0 } },
      // D,E under B (gold)
      ...
      E: { name: 'Rage Shaman', desc: '+45% dmg, +2 AoE, chain heal 3, AURA: +15% speed', damageMult: 1.45, special: { aoeRadiusBonus: 2, healBonus: 5, chainHeal: 3, auraSpeedBonus: 0.15 }, spawnSpeedMult: 0.82, cost: { gold: 90, wood: 0, meat: 0 } },
      // F,G under C (wood)
      F: { name: 'Swift Chanter', desc: '+20% dmg, very fast, AURA: +10% armor', damageMult: 1.20, attackSpeedMult: 0.60, special: { healBonus: 5, auraArmorBonus: 0.10 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 90, meat: 0 } },
      G: { name: 'Doom Chanter', desc: '+65% dmg, AURA: +15% atk speed', damageMult: 1.65, rangeMult: 1.30, special: { auraAttackSpeedBonus: 0.15 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 90, meat: 0 } },
    },
  ...
  [Race.Goblins]: {
    // Melee B-path = wood-heavy (troll tank/bruiser), C-path = gold-heavy (goblin speed/damage)
    [BuildingType.MeleeSpawner]: {
  ...
  [Race.Demon]: {
    ...
    [BuildingType.MeleeSpawner]: {
      ...
      G: { name: 'Hellstrike', desc: '+45% dmg, +25% speed, cleave 2', damageMult: 1.45, moveSpeedMult: 1.25, special: { cleaveTargets: 2 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 55, meat: 30 } },
    },
    ...
    [BuildingType.CasterSpawner]: {
      ...
      F: { name: 'Flame Conduit', desc: 'Very fast, +2 burn', attackSpeedMult: 0.65, special: { extraBurnStacks: 2 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 50, meat: 15 } },
  ...
  [Race.Deep]: {
    ...
    [BuildingType.RangedSpawner]: {
      ...
      D: { name: 'Hammerhead', desc: '+40% dmg, faster atk, splash 2t', damageMult: 1.40, attackSpeedMult: 0.85, special: { splashRadius: 2, splashDamagePct: 0.50 }, spawnSpeedMult: 0.85, cost: { gold: 70, wood: 20, meat: 0 } },
  ...
  [Race.Wild]: {
    ...
    [BuildingType.MeleeSpawner]: {
      ...
      D: { name: 'Minotaur', desc: '+55% HP, +40% dmg, cleave 1', hpMult: 1.55, damageMult: 1.40, special: { cleaveTargets: 1 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 65, meat: 15 } },
      ...
      F: { name: 'Viper Nest', desc: 'Spawn 3 snakes, +35% speed, +2 burn', hpMult: 0.65, damageMult: 0.65, moveSpeedMult: 1.35, special: { spawnCount: 3, extraBurnStacks: 2 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 15, meat: 55 } },
    ...
    [BuildingType.CasterSpawner]: {
      ...
      E: { name: 'Storm Sage', desc: '+40% dmg, +6 heal, +1 AoE', damageMult: 1.40, special: { healBonus: 6, aoeRadiusBonus: 1 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 55, meat: 25 } },
      F: { name: 'Feral Sage', desc: 'Very fast, +2 burn', attackSpeedMult: 0.65, special: { extraBurnStacks: 2 }, spawnSpeedMult: 0.82, cost: { gold: 0, wood: 15, meat: 50 } },
  ...
  [Race.Geists]: {
    [BuildingType.MeleeSpawner]: {
      B: { name: 'Iron Bones', desc: '+40% HP, +25% dmg', hpMult: 1.40, damageMult: 1.25, spawnSpeedMult: 0.90, cost: { gold: 0, wood: 0, meat: 22, souls: 5 } },
      C: { name: 'Ambush Chest', desc: '+25% speed, 25% dodge', moveSpeedMult: 1.25, special: { dodgeChance: 0.25 }, spawnSpeedMult: 0.90, cost: { gold: 39, wood: 0, meat: 0, souls: 5 } },
      D: { name: 'Death Knight', desc: '+50% dmg, +2 burn', damageMult: 1.50, special: { extraBurnStacks: 2 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 } },
      E: { name: 'Soul Eater', desc: '+45% HP/40% dmg, regen 3/s', hpMult: 1.45, damageMult: 1.40, special: { regenPerSec: 3 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 } },
      F: { name: 'Snapping Mimic', desc: '+25% dmg, +35% speed, 35% dodge', damageMult: 1.25, moveSpeedMult: 1.35, special: { dodgeChance: 0.35 }, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 } },
      G: { name: 'Soul Gorger', desc: 'Grows stronger from nearby deaths (max 20)', damageMult: 1.20, hpMult: 1.30, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 }, special: { soulHarvest: true, soulHarvestRadius: 8, soulMaxStacks: 20 } },
    },
    [BuildingType.RangedSpawner]: {
      B: { name: 'Venom Wraith', desc: '+35% dmg, +2 burn', damageMult: 1.35, special: { extraBurnStacks: 2 }, spawnSpeedMult: 0.90, cost: { gold: 39, wood: 0, meat: 0, souls: 5 } },
      C: { name: 'Bone Skull', desc: 'Faster atk, +25% range', attackSpeedMult: 0.80, rangeMult: 1.25, spawnSpeedMult: 0.90, cost: { gold: 0, wood: 0, meat: 22, souls: 5 } },
      D: { name: 'Plague Arrow', desc: '+45% dmg, +3 burn', damageMult: 1.45, special: { extraBurnStacks: 3 }, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 } },
      E: { name: 'Hex Volley', desc: 'Fires 2 projectiles', special: { multishotCount: 1, multishotDamagePct: 0.75 }, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 } },
      F: { name: 'Wailing Skull', desc: '+20% dmg, +30% speed, 25% dodge', damageMult: 1.20, moveSpeedMult: 1.30, special: { dodgeChance: 0.25 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 } },
      G: { name: 'Bone Ballista', desc: 'SIEGE: 13 range, slow, devastating vs buildings', hpMult: 0.50, damageMult: 2.04, attackSpeedMult: 3.20, moveSpeedMult: 0.38, rangeMult: 1.50, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 }, special: { isSiegeUnit: true, buildingDamageMult: 4.0, splashRadius: 3, splashDamagePct: 0.65, extraBurnStacks: 1 } },
    },
    [BuildingType.CasterSpawner]: {
      B: { name: 'Plague Mage', desc: '+30% HP, 15% summon chance', hpMult: 1.30, special: { skeletonSummonChance: 0.15 }, spawnSpeedMult: 0.90, cost: { gold: 0, wood: 0, meat: 22, souls: 5 } },
      C: { name: 'Dark Sorcerer', desc: 'Faster atk, +25% range', attackSpeedMult: 0.80, rangeMult: 1.25, spawnSpeedMult: 0.90, cost: { gold: 39, wood: 0, meat: 0, souls: 5 } },
      D: { name: 'Necromancer', desc: '+40% dmg, 18% summon chance', damageMult: 1.40, special: { skeletonSummonChance: 0.18 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 } },
      E: { name: 'Soul Harvester', desc: '+30% dmg, 25% summon, +2 AoE', damageMult: 1.30, special: { skeletonSummonChance: 0.25, aoeRadiusBonus: 2 }, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 } },
      F: { name: 'Shadow Sorcerer', desc: '+20% dmg, very fast, 25% lifedrain', damageMult: 1.20, attackSpeedMult: 0.65, special: { lifeDrainPct: 0.25 }, spawnSpeedMult: 0.85, cost: { gold: 84, wood: 0, meat: 0, souls: 10 } },
      G: { name: 'Arch Lich', desc: '+65% dmg, applies vulnerable, +25% range', damageMult: 1.65, rangeMult: 1.25, special: { applyVulnerable: true }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 42, souls: 10 } },
    },
  ...
  [Race.Tenders]: {
    [BuildingType.MeleeSpawner]: {
      ...
      D: { name: 'Elder Ent', desc: '+45% HP, +20% dmg, regen 2/s', hpMult: 1.45, damageMult: 1.20, spawnSpeedMult: 0.85, special: { regenPerSec: 2 }, cost: { gold: 0, wood: 90, meat: 0 } },
    ...
    [BuildingType.RangedSpawner]: {
      ...
      E: { name: 'Grand Tinker', desc: '+40% dmg, +30% range, +2 slow', damageMult: 1.40, rangeMult: 1.30, special: { extraSlowStacks: 2 }, spawnSpeedMult: 0.85, cost: { gold: 90, wood: 0, meat: 0 } },
    ...
    [BuildingType.CasterSpawner]: {
      B: { name: 'Deep Root', desc: '+35% HP, +20% dmg, +5 heal', hpMult: 1.35, damageMult: 1.20, special: { healBonus: 5 }, spawnSpeedMult: 0.90, cost: { gold: 0, wood: 0, meat: 45 } },
      ...
      E: { name: 'Bloom Shaper', desc: '+50% dmg, +2 AoE, +5 heal', damageMult: 1.50, special: { aoeRadiusBonus: 2, healBonus: 5 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 0, meat: 90 } },
      F: { name: 'Mycelium Sage', desc: 'Very fast, +4 slow, chain 2', attackSpeedMult: 0.65, special: { extraSlowStacks: 4, extraChainTargets: 2 }, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 90, meat: 0 } },
      G: { name: 'Fungal Lord', desc: '+65% dmg, +35% range', damageMult: 1.65, rangeMult: 1.35, spawnSpeedMult: 0.85, cost: { gold: 0, wood: 90, meat: 0 } },
    },
...
export const RACE_RESEARCH_UPGRADES: Record<Race, ResearchUpgradeDef[]> = {
  ...
  [Race.Tenders]: [
    ...
    { id: 'tenders_caster_2', category: 'caster', type: 'race_special', name: 'Life Link', desc: '{healing} Double heal <30% HP', oneShot: true },
  ],
};
...
export const RACE_ABILITY_UPGRADES: Record<Race, ResearchUpgradeDef[]> = {
  ...
  [Race.Oozlings]: [
    ...
    { id: 'oozlings_ability_2', category: 'ability', type: 'race_ability', name: 'Caster Mound', desc: '{spawn-rate} 25% chance to spawn caster ooze', oneShot: true },
    { id: 'oozlings_ability_3', category: 'ability', type: 'race_ability', name: 'Death Burst', desc: '{explode} Mound spawns 3 random ooze on death', oneShot: true },
    { id: 'oozlings_ability_4', category: 'ability', type: 'race_ability', name: 'Ooze Vitality', desc: '{regen} All Oozling units regenerate 2 HP/s', oneShot: true },
  ],
  [Race.Demon]: [
  ...
  [Race.Deep]: [
    ...
    { id: 'deep_ability_2', category: 'ability', type: 'race_ability', name: 'Healing Rain', desc: '{healing} Deluge heals allies 5 HP/sec', oneShot: true },
    { id: 'deep_ability_3', category: 'ability', type: 'race_ability', name: 'Freezing Depths', desc: '{slow} Slowed units move 15% slower', oneShot: true },
  ...
  if (def.oneShot) {
    ...
    if (def.type === 'race_ability') {
      const abilityCosts: Record<string, { gold: number; wood: number; meat: number; souls?: number; deathEssence?: number }> = {
        ...
        geists_ability_4: { gold: 300, wood: 0, meat: 0, souls: 80 }, // Hungering Dark: lifesteal=+dmg — 310 eff, elite
